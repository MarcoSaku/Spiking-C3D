...
from nengo_ocl.utils import as_ascii, indent, round_up, round_up2
...
def plan_presentinput_3D(queue, Y, t, signals, dt, pres_t=None, tag=None):
    N = len(Y)
    assert len(Y) == len(t) == len(signals)
    assert pres_t is None or pres_t.shape == (N,)
    ...
    _fn = cl.Program(queue.context, text).build().presentinput_3D
    _fn.set_args(*[arr.data for arr in full_args])
    ...
    lsize = (max_len, 1)
    rval = Plan(
        queue, _fn, gsize, lsize=lsize, name="cl_presentinput_3D", tag=tag)
    rval.full_args = full_args     # prevent garbage-collection
    return rval
...
def plan_conv2(queue, X, Y, filters, biases, shape, conv, tag=None,
    ...
    max_group = min(queue.device.max_work_group_size, 1024)
    assert max_group >= 32
    lsize0 = min(nj, 32)
...
'''
3D Matrix Indexing:
matrix_3d[nd][ni][nj] - indices: d, i, j
matrix_3d[d][i][j]=matrix_3d[(d*ni*nj) + (i*nj) + j]
...
def plan_conv3(queue, X, Y, filters, biases, shape, conv, tag=None,
               transposed=False):
    """
    Parameters
    ----------
    ...
    )
    {
        ...
        const int dij = (d*${ni}*${nj}) + (i*${nj}) + j;

        const int td = get_local_id(0);
        const int ti = get_local_id(1);
        const int tj = get_local_id(2);
        const int lsized = get_local_size(0);
        const int lsizei = get_local_size(1);
        const int lsizej = get_local_size(2);
        const int lsize = lsizei * lsizej * lsized;
        const int tdij = (td * lsizej * lsizei) + (ti * lsizej) + tj;
        const int d0 = d - td;
        const int i0 = i - ti;
        const int j0 = j - tj;
        __local ${type} patch[${ndpatch}][${nipatch}][${njpatch}];
    % if conv:
        __local ${type} filters[${sdij} * ${nf}];
    % else:
        f += dij;
    % endif
        x += ${xstart};
    ...
    % if conv or nf_per >= nf:
        ...
        for (int k = 0; k < ${nf}; k++)
            outs[k] = b[k*${ndij} + dij];
    % else:
        const int k0 = get_global_id(3);
        const int ksize = get_global_size(3);

        ${type} outs[${nf_per}];
        for (int k = 0, kk = k0; k < ${nf_per}; k++, kk += ksize)
            outs[k] = (kk < ${nf}) ? b[kk*${ndij} + dij] : 0;
    % endif

        for (int c = 0; c < ${nc}; c++) {

            // load image section
            __global const ${type} *xc = &x[c * ${ndij}];
            for (int k = tdij; k < ${npatch}; k += lsize) {
                const int kd = k / (${nipatch}*${njpatch});
                const int ki = (k / ${njpatch})%${nipatch};
                const int kj = k % ${njpatch};
                const int ii = i0 + ki - ${si2};
                const int jj = j0 + kj - ${sj2};
                const int dd = d0 + kd - ${sd2};
                if (ii >= 0 && ii < ${ni} && jj >= 0 && jj < ${nj} && dd>=0 && dd < ${nd})
                    patch[kd][ki][kj] = xc[(ii*${nj}) + jj + (dd * ${nj} * ${ni})];
                else
                    patch[kd][ki][kj] = 0;
            }
    ...
    % if conv:
            // load filters
            __global const ${type} *fc = f + c*${sdij}*${nf};
            for (int k = tdij; k < ${sdij} * ${nf}; k += lsize) {
                filters[k] = fc[k];
            }
    % endif
            ...
            for (int ii = 0; ii < ${si}; ii++) {
            for (int jj = 0; jj < ${sj}; jj++) {
                const ${type} xcdij = patch[td+dd][ti+ii][tj+jj];
    % if conv:
                __local const ${type} *fcdij = &filters[((ii*${sj}) + jj + (dd*${sj}*${si}))*${nf}];
    % else:
                __global const ${type} *fcdij =
                    &f[(c*${sdij} + (ii*${sj}) + jj + (dd*${sj}*${si}) )*${nf}*${ndij}];
    % endif

    % if conv:
                for (int k = 0; k < ${nf}; k++)
                    outs[k] += fcdij[k] * xcdij;
    % elif nf_per >= nf:
      for (int k = 0; k < ${nf}; k++)
                    outs[k] += fcij[k*${ndij}] * xcdij;
    % else:
        for (int k = 0, kk = k0; k < ${nf_per}; k++, kk += ksize)
            outs[k] += (kk < ${nf}) ? fcdij[kk*${ndij}] * xcdij : 0;
    % endif
            }
            }
            }
    ...
    % else:
            for (int k = 0, kk = k0; k < ${nf_per}; k++, kk += ksize)
                if (kk < ${nf})
                    y[kk*${ndij} + dij] = outs[k];
    % endif
        }
    }
    """

    nf, nd, ni, nj, nc, sd, si, sj = shape
    nf_per = 16

    max_group = min(queue.device.max_work_group_size, 700)
    assert max_group >= 32
    # lsize0 = min(nj, 32)
    # lsize1 = min(max_group // lsize0, ni)
    lsize0 = min(nd, 8)
    lsize1 = min(8, ni)
    lsize2 = min(max_group // (lsize0*lsize1), nj)
    lsize = (lsize0, lsize1)
    lsize=lsize + (lsize2,)
    #lsize=(8,8,8)
    #lsize=(2,2,2)
    gsize = (round_up(nd, lsize[0]),round_up(ni, lsize[1]), round_up(nj, lsize[2]))
    if not conv and nf_per < nf:
        lsize = lsize + (1,)
        gsize = gsize + (int(np.ceil(nf / nf_per)),)
    ...
    sdij = si * sj * sd
    csdij = nc * sdij
    sd2, si2, sj2 = (sd - 1) // 2,(si - 1) // 2, (sj - 1) // 2
    ndpatch = lsize[0] + sd - 1
    nipatch = lsize[1] + si - 1
    njpatch = lsize[2] + sj - 1
    npatch = nipatch * njpatch * ndpatch
    #print 'Shape:'+str(shape) + ' lsize:'+str(lsize)+' gsize:'+str(gsize)+' npatch:'+str(npatch)

    assert np.prod(lsize) <= queue.device.max_work_group_size
    assert npatch <= queue.device.local_mem_size / X.dtype.itemsize
    textconf = dict(
        type=X.ctype, conv=conv, nf=nf, ni=ni, nj=nj, nd=nd, nc=nc, si=si, sj=sj,sd=sd,
        nf_per=nf_per, ndij=ndij, sdij=sdij, csdij=csdij, si2=si2, sj2=sj2, sd2=sd2,
        nipatch=nipatch, njpatch=njpatch, ndpatch=ndpatch, npatch=npatch,
        xstart=X.start, ystart=Y.start)
    text = as_ascii(Template(text, output_encoding='ascii').render(**textconf))

    full_args = (X.base_data, filters.data, biases.data, Y.base_data)
    _fn = cl.Program(queue.context, text).build().conv3
    _fn.set_args(*full_args)

    rval = Plan(queue, _fn, gsize, lsize=lsize, name="cl_conv3", tag=tag)
    rval.full_args = full_args     # prevent garbage-collection
    rval.flops_per_call = 2 * nd *ni * nj * nf * nc * si * sj * sd
    rval.bw_per_call = X.nbytes + filters.nbytes + biases.nbytes + Y.nbytes
    return rval
...
'''
def plan_pool3(queue, X, Y, shape, size, depth_size, stride, temporal_stride, tag=None):
    for ary in [X, Y]:
        # assert that arrays are contiguous
        assert len(ary.shape) in [1, 2]
        assert ary.strides[-1] == ary.dtype.itemsize
    ...
    )
    {
        ...
        const int lsizei = get_local_size(1);
        const int lsizej = get_local_size(2);
        const int lsize = lsized * lsizei * lsizej;
        const int tdij = (td * lsizej * lsizei) + (ti * lsizej) + tj;
        const int d0 = d - td;
        const int i0 = i - ti;
        const int j0 = j - tj;
        ...
        // load image patch
        __global const ${type} *xc = &x[c * ${nxd * nxi * nxj}];
        for (int k = tdij; k < ${ndpatch * nipatch * njpatch}; k += lsize) {
            const int kd = k / (${nipatch}*${njpatch});
            const int ki = (k / ${njpatch})%${nipatch};
            const int kj = k % ${njpatch};
            const int dd = d0*${temporal_st} + kd;
            const int ii = i0*${st} + ki;
            const int jj = j0*${st} + kj;
            if (ii >= 0 && ii < ${nxi} && jj >= 0 && jj < ${nxj} && dd >= 0 && dd < ${nxd})
                patch[kd][ki][kj] = xc[(ii*${nxj}) + jj + (dd *${nxj} * ${nxi})];
            else
                patch[kd][ki][kj] = NAN;
        }
        ...
        int n = 0;
        ${type} xdij;
        for (int dd = 0; dd < ${depth_s}; dd++)
        for (int ii = 0; ii < ${s}; ii++) {
        for (int jj = 0; jj < ${s}; jj++) {
            xdij = patch[td*${temporal_st} + dd][ti*${st} + ii][tj*${st} + jj];
            if (!isnan(xdij)) {
                out += xdij;
                n++;
            }
        ...
        if (i < ${nyi} && j < ${nyj} && d < ${nyd})
            y[c*${nyi * nyj * nyd} + i*${nyj} + j + (d * ${nyj} * ${nyi})] = out / n;
    }
    """
    nc, nyd, nyi, nyj, nxd, nxi, nxj = shape
    ...
    gsize = (round_up(nyd, lsize[0]), round_up(nyi, lsize[1]), round_up(nyj, lsize[2]), nc)

    ndpatch = lsize[0]*temporal_stride + depth_size - 1
    nipatch = lsize[1]*stride + size - 1
    njpatch = lsize[2]*stride + size - 1
    assert nipatch*njpatch*ndpatch <= queue.device.local_mem_size / X.dtype.itemsize


    textconf = dict(
        type=X.ctype, Xstart=X.start, Ystart=Y.start,
        nxi=nxi, nxj=nxj,nxd=nxd, nyi=nyi, nyj=nyj,nyd=nyd, s=size,depth_s=depth_size, st=stride,temporal_st=temporal_stride,
        nipatch=nipatch, njpatch=njpatch,ndpatch=ndpatch)

    text = as_ascii(Template(text, output_encoding='ascii').render(**textconf))

    full_args = (X.base_data, Y.base_data)
    _fn = cl.Program(queue.context, text).build().pool3
    _fn.set_args(*full_args)

    rval = Plan(queue, _fn, gsize, lsize=lsize, name="cl_pool3", tag=tag)
    rval.full_args = full_args     # prevent garbage-collection
    rval.flops_per_call = X.size
    rval.bw_per_call = X.nbytes + Y.nbytes
    return rval
...
def plan_pool3(queue, X, Y, shape, size, depth_size, stride, temporal_stride, tag=None):
    ...
    lsize0 = min(nxd, 8)
    lsize1 = min(8, nxi)
    lsize2 = min(max_group // (lsize0*lsize1), nxj)
    lsize = (lsize0, lsize1)
    lsize=lsize + (lsize2,)
    #lsize=(8,8,8)

    gsize = (round_up(nyd, lsize[0]), round_up(nyi, lsize[1]), round_up(nyj, lsize[2]))
    ...
    textconf = dict(
        type=X.ctype, Xstart=X.start, Ystart=Y.start,
        nxi=nxi, nxj=nxj,nxd=nxd, nyi=nyi, nyj=nyj,nyd=nyd, s=size,depth_s=depth_size, st=stride,temporal_st=temporal_stride,
        nipatch=nipatch, njpatch=njpatch,ndpatch=ndpatch, nc=nc)
