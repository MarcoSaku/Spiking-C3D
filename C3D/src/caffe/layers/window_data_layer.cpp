...
void WindowDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  (*top)[0]->Reshape(batch_size, channels, 1, crop_size, crop_size);
  prefetch_data_.reset(
      new Blob<Dtype>(batch_size, channels, 1, crop_size, crop_size));
  ...
  prefetch_label_.reset(
      new Blob<Dtype>(batch_size, 1, 1, 1, 1));
  ...
  if (this->layer_param_.window_data_param().has_mean_file()) {
    ...
    data_mean_.Reshape(1, channels, 1, crop_size, crop_size);
  }
