...
__global__ void LRNFillScale(const int nthreads, const Dtype* in,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int n = index / width / height / length;
    int offset = ((n * channels * length + l) * height + h) * width + w;
    int step = length * height * width;
    in += offset;
    scale += offset;
    int head = 0;
...
void LRNLayer<Dtype>::CrossChannelBackward_gpu(
  ...
  LRNComputeDiff<<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      ...
      scale_.gpu_data(), top[0]->gpu_diff(), num_, channels_, length_, height_, width_,
