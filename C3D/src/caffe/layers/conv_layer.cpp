...
void ConvolutionLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  col_buffer_.Reshape(
      1, channels_ * kernel_size_ * kernel_size_, 1, height_out, width_out);
  // Set the parameters
  ...
  (*top)[0]->Reshape(bottom[0]->num(), num_output_, 1, height_out, width_out);
  ...
  if (this->blobs_.size() > 0) {
    ...
    this->blobs_[0].reset(new Blob<Dtype>(
        num_output_, channels_ / group_, 1, kernel_size_, kernel_size_));
    // fill the weights
